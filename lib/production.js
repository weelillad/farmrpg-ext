...
const renderDaily = async state => {
    ...
    // Chicken Coop.
    if (state.player.coop !== null) {
        rows.push(await renderProductionRow(state, "Eggs", state.player.coop.eggProd, `coop.php?id=${state.player.farmID}`))
        rows.push(await renderProductionRow(state, "Feathers", state.player.coop.featherProd, `coop.php?id=${state.player.farmID}`))
    }
    // Cow Pasture.
    if (state.player.pasture !== null) {
        rows.push(await renderProductionRow(state, "Milk", state.player.pasture.milkProd, `pasture.php?id=${state.player.farmID}`))
    }
